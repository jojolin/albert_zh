...
flags.DEFINE_string(
    ...
    "the base dir to export model for serving")
...
class RemyProcesser(DataProcessor):
    """ Processor for remy data set """

    def __init__(self):
        self.language = "zh"

    def load_examples(self, data_dir, usage):
        examples = []
        with open(os.path.join(data_dir, usage), 'r', encoding='utf8') as r:
            lines = r.readlines()
            for (i, line) in enumerate(lines):
                ...
                if line.startswith("text_a"):
                    continue
                if usage.startswith('test') and line.find('\x02') == -1:
                    # case: 单个测试句子1行
                    sen = list(line.strip())
    ...
    def get_train_examples(self, data_dir):
        return self.load_examples(data_dir, 'train.tsv')

    def get_dev_examples(self, data_dir):
        return self.load_examples(data_dir, 'dev.tsv')

    def get_test_examples(self, data_dir):
        return self.load_examples(data_dir, 'test.tsv')
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      ...
      for (i, zip_preds) in enumerate(zip(result, predict_examples)):
        prediction, pred_example = zip_preds
  ...
  if FLAGS.do_export:    # export model for serving
    ...
    # solve issue: Operation of type AssignVariableOp (AssignVariableOp) is not supported on the TPU for inference
    estimator._export_to_tpu = False 
    export_base_dir = FLAGS.export_dir_base if FLAGS.export_dir_base else './export_serving'
    estimator.export_savedmodel(export_base_dir, serving_input_receiver_fn) 
    print('export to {}'.format(export_base_dir))
